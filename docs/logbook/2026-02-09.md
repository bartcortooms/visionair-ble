...
Observed blocker:
- `vmictl` navigation commands (`measurements-full`) hang waiting for UI selectors
- Preflight shows top resumed activity is launcher (`com.android.launcher3/...`) rather than VMI+
- `vmictl launch` returns successfully but device remains on launcher UI in subsequent dumps

Action taken:
- Added `scripts/capture/preflight_capture.py` to check connectivity, keyguard state, and top resumed activity before running long capture flows
